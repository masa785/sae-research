...
def off_diagonal_sparsity_loss(B):
    """B の非対角成分のみにL1正則化。対角成分は保護する。"""
    h_dim = B.shape[0]
    eye = torch.eye(h_dim, device=B.device)
    off_diag = B * (1 - eye)
    return torch.abs(off_diag).sum()
...
with tqdm(total=total_steps) as pbar:
    for step in range(total_steps):
        try:
            ...
            B_sparsity_loss = off_diagonal_sparsity_loss(model.B)
...
if DirectLiNGAM is not None:
    ...
    eps_all = np.concatenate(eps_collected, axis=0)
    ...
    print(f"\n[診断] 推定ノイズ間の平均絶対相関: {off_diag_corr:.4f}")
    lingam_reliable = off_diag_corr < independence_threshold
    if not lingam_reliable:
        print(f"[WARNING] ノイズ独立性が閾値{independence_threshold}を超過。LiNGAM結果の信頼性は低い可能性。")
